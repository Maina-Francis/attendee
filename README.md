...
- For local development with local file storage:
  - Create a `.env.local` file based on your `.env` file
  - Set `DJANGO_SETTINGS_MODULE=attendee.settings.local` in your `.env.local` file
  - You can leave the AWS variables empty as files will be stored locally
